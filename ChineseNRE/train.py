...
with open('./data/processed_data/entity_gingko_train.pkl', 'rb') as inp:
...
for epoch in range(EPOCHS):
    ...
    for sentence, pos1, pos2, tag in train_dataloader:
        ...
        # sentence是句子
        # tag是标签
        # y是预测结果，是一个128×12的tensor，每一个长度为12的向量都是对这个句子的12中可能关系的打分，下面再取最大值就可以得到最有可能的预测结果
        ...
        # y已经转变成了真实的预测结果，是一个长度为128的array，存着对128个句子的预测
    ...
    acc_t = 0
    total_t = 0
    count_predict = [0, 0, 0, 0, 0]   # count_predict是预测的情况
    count_total = [0, 0, 0, 0, 0]     # count_total是实际的情况
